...
const page = () => {
  ...
  useEffect(() => {
    ...
    const sendRequest = async () => {
      try {
        const response = await axios.get(`${baseURL}/banner/${params.id}`, {
          headers: {
...
const {
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (id !== "add") {
          response = await axios.post(`${baseURL}/update_banner/${data.id}`, formData, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        } else {
          response = await axios.post(`${baseURL}/banner`, formData, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
